...
def solve_cvrp(
    ...
    # Chỉ cần "nới lỏng" cumul-start (fix_start_cumul_to_zero=False) và cho phép
    # slack (chờ đợi) lớn KHI thực sự dùng VRPTW — vì lúc đó mỗi xe có thể cần
    # xuất phát muộn hơn 0 hoặc chờ tới khi time window của điểm tiếp theo mở.
    # Với CVRP thường (không time window), KHÔNG được để start tự do: cumul
    # start phải cố định = 0 và slack = 0, nếu không biến thời gian bắt đầu sẽ
    # tự do trong cả khoảng [0, horizon], làm không gian tìm kiếm phình to
    # không cần thiết và khiến OR-Tools/GLS không kịp tìm ra lời giải trong
    # time_limit_sec (báo "infeasible" giả, dù bài toán thực chất có lời giải).
    use_tw = bool(config.use_time_windows and config.time_windows_s)
    slack_max = int(horizon) if use_tw else 0
    fix_start_cumul_to_zero = not use_tw

    routing.AddDimension(
        time_cb_index,
        slack_max,                  # slack tối đa (chỉ cần khi có time window)
        int(horizon),                # thời gian tuyến tối đa mỗi xe
        fix_start_cumul_to_zero,     # ép cumul start = 0 khi KHÔNG dùng time window
        "Time",
    )
    ...
    if use_tw:
        ...
        # Không time window: vẫn nên "kéo" cumul về giá trị nhỏ nhất khả thi để
        # solver hội tụ nhanh hơn (dù start đã fix=0, finalizer giúp end_cumul
        # được tối thiểu hoá thay vì để tự do tới tận horizon).
        for v in range(config.num_vehicles):
