...
from forecasting import generate_waste_history, train_xgboost_forecast, predict_next_days_xgboost
...
with st.sidebar:
    ...
    st.header("5. AI dự báo nhu cầu rác (XGBoost)")
    use_ml_forecast = st.checkbox(
        "Dùng XGBoost dự báo khối lượng rác cho ngày thu gom", value=True,
        help="Sinh lịch sử 180 ngày mô phỏng theo từng điểm, train/test theo thời gian và dự báo ngày kế tiếp. Nếu tắt, app dùng waste_kg trong dữ liệu đầu vào."
    )
    forecast_history_days = st.slider("Số ngày lịch sử dự báo", 60, 365, 180, step=30)
    st.caption("XGBoost là mô hình Machine Learning; kết quả dự báo được đưa vào demand của OR-Tools.")
...
if run_btn:
    ...
    # ---------------- MACHINE LEARNING: XGBOOST FORECAST ----------------
    ml_forecast_dict = None
    ml_forecast_meta = None
    demands = df_points["waste_kg"].tolist()
    if use_ml_forecast:
        with st.spinner("Đang dự báo khối lượng rác bằng XGBoost (Machine Learning)..."):
            try:
                history = generate_waste_history(
                    df_points, days=int(forecast_history_days), seed=int(seed) if data_mode == "Dữ liệu demo (Lê Văn Việt)" else 42
                )
                cutoff = (history["date"].max() - pd.Timedelta(days=14)).strftime("%Y-%m-%d")
                result_types = {}
                mae_rows = []
                for waste_type in ["organic", "recyclable", "other"]:
                    target = f"waste_{waste_type}_kg"
                    xgb_result = train_xgboost_forecast(history, target, cutoff)
                    future = predict_next_days_xgboost(history, target, xgb_result, num_days=1)
                    for _, rr in future.iterrows():
                        result_types.setdefault(rr["node_id"], {})[waste_type] = float(rr["predicted_kg"])
                    mae_rows.append({"Loại rác": waste_type, "MAE test (kg)": round(float(xgb_result.mae), 2)})

                ml_forecast_dict = {}
                for nid, vals in result_types.items():
                    ml_forecast_dict[nid] = {**vals, "total_kg": round(sum(vals.values()), 1)}

                demands = [
                    0.0 if bool(row["is_depot"]) else float(ml_forecast_dict.get(row["node_id"], {}).get("total_kg", row["waste_kg"]))
                    for _, row in df_points.iterrows()
                ]
                ml_forecast_meta = {"history_days": int(forecast_history_days), "cutoff": cutoff, "mae": mae_rows}
                st.success("XGBoost đã dự báo demand và demand dự báo được đưa vào OR-Tools.")
            except Exception as exc:
                st.error(f"XGBoost forecast thất bại: {exc}")
                st.warning("App quay về waste_kg trong dữ liệu đầu vào để pipeline vẫn chạy.")
                ml_forecast_dict = None
    ...
    st.session_state.results = {
        ...
        "ml_forecast_dict": ml_forecast_dict,
        "ml_forecast_meta": ml_forecast_meta,
    }
...
if res.get("ml_forecast_dict") is not None:
    st.subheader("🤖 Machine Learning: XGBoost dự báo khối lượng rác")
    meta = res.get("ml_forecast_meta") or {}
    st.caption(
        f"Dữ liệu lịch sử mô phỏng: {meta.get('history_days', '-')} ngày; "
        f"train/test chia theo thời gian, cutoff = {meta.get('cutoff', '-')}. "
        "Dự báo ngày kế tiếp được dùng làm demand cho bài toán định tuyến."
    )
    mae_df = pd.DataFrame(meta.get("mae", []))
    if not mae_df.empty:
        st.dataframe(mae_df, use_container_width=True, hide_index=True)
    forecast_rows = []
    for _, row in df_points[~df_points["is_depot"]].iterrows():
        f = res["ml_forecast_dict"].get(row["node_id"], {})
        forecast_rows.append({
            "Điểm": row["node_id"],
            "Waste_kg hiện tại": round(float(row["waste_kg"]), 1),
            "Organic dự báo": round(float(f.get("organic", 0)), 1),
            "Recyclable dự báo": round(float(f.get("recyclable", 0)), 1),
            "Other dự báo": round(float(f.get("other", 0)), 1),
            "Tổng demand dự báo": round(float(f.get("total_kg", 0)), 1),
        })
